...
| 检测项 | 判定 | 路由建议 |
|------|------|---------|
| 已有脚手架 + 新需求意图 | `docs/prd/requirements/index.md` 已存在，且用户描述的是**与当前不同的新需求**（须先于下方 PRD 候选行判定，否则不可达） | 路由 `/ms-prd`，由其[新建需求门禁](../ms-prd/SKILL.md#新建需求门禁强约束)⚠️三选一（继续当前 / clear 清理后新建 / discard 后新建）；未确认 ⛔ 不覆盖 |
...
> **补充说明**：dev-workflow 批量模式内部会调用 `/ms-test-run --trace` 执行全量测试 + 追溯验证，详见 `skills/ms-dev-workflow/SKILL.md`。
