...
```bash
# 整仓对齐（推荐日常）
/ms-pipeline realign                     # 扫描全部产物 → 调度各 skill 补齐 → 汇总
/ms-pipeline realign --dry-run           # 只出报告，不动文件

# 定向对齐
/ms-feature F-01 realign                 # 只对齐 F-01 关联产物
/ms-bugfix BUG-05 realign                # 只对齐 BUG-05 关联产物
```
...
| `prd-mapping` | `/ms-pipeline realign --scope=prd-mapping` | 沿用 `skills/ms-prd/references/governance/prd-devdocs-mapping.md` | PRD ↔ DevDocs 映射对齐 |
...
3. **调度**：按三段式顺序依次调用底层 `--realign`（上游 → 主链路 → 旁路）：
   ```
   Phase 1（B 类上游，若文件存在）:
     ms-prd-parser → ms-prd-brainstorm
...
- **bump 触发**：skill 作者在修改产物必填章节/字段/结构时，同步更新 `references/realign.md` 顶部常量 + 填写 Migration Matrix 的对应条目。未同步 = spec 漂移，ms-verify --schema-drift（阶段 3）会报。

## 向后兼容（无元数据的旧产物）

- 阶段 1（纯 flag，无 frontmatter）：产物无 `spec_version` 字段 → 由用户显式 `/ms-pipeline realign` 触发，skill 假设产物 = legacy，直接按当前规范做全量差距扫描。
- 阶段 2（有 frontmatter）：无字段 → `spec_version: legacy` → 归 `ms-retrofit` 处理（大规模迁移），不由 realign 直接重写。
- 阶段 3（诊断入口）：`/ms-verify --schema-drift` 扫描所有产物并报告 legacy / drift / current 三态。

## Dry-run 样例（首次调用 vs 二次调用）

> **说明**：以下为**示意**样例，用于展示报告格式与幂等性，非实际运行结果。`design.v1 → design.v2` 已是真实演进（2026-08-31，§0 摘要节，见 [system-design/references/realign.md](../../ms-system-design/references/realign.md)）；其余 skill 的 `spec_version` 常量仍为 v1 首版、Migration Matrix 仍为"保留字段，未来启用"。本仓尚未真实触发过 realign 执行。
...
### 首次调用 `/ms-pipeline realign --dry-run`
...
- /ms-system-design --realign  # 补齐 2 项 additive（§15 设计审查"原则校验"列、ADR"升级原因"条目）
- /ms-retrofit docs/devdocs/00-context.md  # legacy 产物先由 retrofit 加 frontmatter
- （04-dev-tasks.md 无需动作）

未执行任何修改（dry-run）。运行 /ms-pipeline realign 实际执行。
```
...
|---|---|---|---|---|---|
| 2026-04-22T10:30+08:00 | design.v1 | design.v2 | SOLID 校验表、ADR"升级原因" | — | /ms-pipeline realign |
```
...
- ❌ **不**修改已合规段落（只追加/重构差距项）
- ❌ **不**并入 ms-dev-workflow 的 12 种续做信号（语义污染）
...
`ms-pipeline` 阶段检测完成、返回路由建议之前，执行 schema drift 轻量检查。
...
1. 检查 .devdocs-realign-ack 标记
     ...
     └── 不存在或失效 → 继续 step 2
     │
2. 委托 /ms-verify --schema-drift（只读扫描，≤2s）
     │
...
ℹ️ 规范升级提示
   ...
   建议：
     /ms-pipeline realign --dry-run    先预览差距
     /ms-pipeline realign              按新规范查漏补缺
     /ms-pipeline realign --no-realign 跳过本次（记录决策，不再提示）
...
- 写入方：用户执行 `/ms-pipeline realign` 或 `--no-realign` 后，由编排层写入（非 pipeline 入口本身）
...
`ms-codebase-insight` 有独立的 `schema_version + commit_hash` 失效机制，**不纳入** realign 编排。但 `/ms-verify --schema-drift`（阶段 3）扫描时会读取其元数据并统一报告。
