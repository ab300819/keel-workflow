...
| `MD003` | `heading-style` | 标题风格（ATX / Setext）须统一 | 否（官方未标注，见 4.1「fixable 状态存疑」） |
...
| fixable 状态存疑 | `MD003` | 早期抓取的规则摘要表把 `MD003` 标为 fixable。经核对 markdownlint 官方 `doc/Rules.md` 原文（`MD003` 章节不含 `Fixable:` 声明）与官方配置 schema，确认 `MD003` **不是** fixable——摘要表的标注有误。此条目原为「存疑待核实」，现已核实并更正 |
