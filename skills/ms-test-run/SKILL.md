...
- [ ] **追溯验证必须覆盖所有 AC**
- [ ] ⛔ **不检查 `CON-XXX`**——本 skill 的作用域是**测试套件**。CON 的 `档 = 可执行` 验证方式（守卫脚本 / 命令 / 基准）由 `/ms-dev-workflow` 在 S5 红验 / S6 绿验直接执行，`/ms-verify --impl` B1 复核；只有当某条 CON 的验证方式**引用了测试编号**（如 `IT-012`）时，该测试才按普通用例进入本 skill 范围。**这是有意的边界，不是遗漏。**
- [ ] **未覆盖的 AC 必须明确标记**
- [ ] **文档测试编号无对应实际用例时必须警告**
