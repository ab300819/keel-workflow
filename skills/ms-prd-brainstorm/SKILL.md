...
**最常见用法**: `/ms-prd`（推荐入口，自动调度 brainstorm）
...
/ms-prd-brainstorm --realign      → 规范升级回扫：FR/NFR 结构差距补齐（不新增/修改业务内容）。复用 [共享契约](../ms-pipeline/references/realign.md)；推荐 `/ms-pipeline realign`
```
...
> "继续当前需求"追加块时续编、不撞旧号；"新需求"（脚手架已清）自然从 01 重起 —— 产品阶段编号是映射进 DevDocs 后即作废的临时标识，无需跨需求全局唯一。`docs/prd/` 是单需求一次性脚手架，详见 [ms-prd 单需求脚手架原则](../ms-prd/SKILL.md#单需求脚手架原则)。
