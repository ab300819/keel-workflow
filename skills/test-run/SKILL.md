...
**最常见用法**: `/test-run`（全量）、`/test-run --ut`（仅单元测试）

**不适合?** 设计测试用例→`/test-cases`
...
- `/dev-workflow` 批量完成后**建议**调用（`--trace`）、单任务完成后**必须**调用（`--affected`）——以 [dev-workflow 全量测试验证约束](../dev-workflow/SKILL.md) 为准
...
```
/test-run              # 全量执行（UT + IT + E2E）
/test-run --ut         # 仅单元测试
/test-run --it         # 仅集成测试
/test-run --e2e        # 仅 E2E 测试
/test-run F-001        # 按功能点关联的测试
/test-run --trace      # 全量执行 + 追溯验证
/test-run --affected   # 仅运行受变更影响的测试
/test-run --affected --ut  # 受影响的单元测试
```
...
> 测试命令在 `workspace_context.code_roots` 各路径下执行（`inline` 时为单项，`path` = 仓库根绝对路径，**调用方不分支**）。代码根多于一项时逐个执行并在报告中按 root 分组。测试报告恒写 `<workspace_context.docs_dir>/devdocs/05-test-report.md`。代码根取自 握手 `workspace_context`（[shared/constraints.md](../shared/constraints.md) §3 `task/workspace-context`；未传入时按 `inline` 缺省）。
...
| 用例覆盖 | 文档中的测试编号在实际测试中有对应用例 | ⚠️ 用例缺失 |
...
| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 测试用例来源 | `/test-cases` | 前置：提供 UT/IT/E2E 用例清单和追溯矩阵 |
| 测试质量标准 | `/testing-guide` | 协作：覆盖率和断言质量判定标准 |
| 开发完成后调用 | `/dev-workflow` | 被调用：批量开发完成后自动触发全量测试 |
| 追溯更新 | `/sync` | 后续：测试结果可触发追溯矩阵状态更新 |
...
- [ ] **追溯验证必须覆盖所有 AC**
- [ ] ⛔ **不检查 `CON-XXX`**——本 skill 的作用域是**测试套件**。CON 的 `档 = 可执行` 验证方式（守卫脚本 / 命令 / 基准）由 `/dev-workflow` 在 S5 红验 / S6 绿验直接执行，`/verify --impl` B1 复核；只有当某条 CON 的验证方式**引用了测试编号**（如 `IT-012`）时，该测试才按普通用例进入本 skill 范围。**这是有意的边界，不是遗漏。**
- [ ] **未覆盖的 AC 必须明确标记**
- [ ] **文档测试编号无对应实际用例时必须警告**
- [ ] **追溯结果不修改源文件**（仅报告，修改由 /sync 负责）
