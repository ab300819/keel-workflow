...
**最常见用法**: `/onboard`（自动检测 read/update）

**不适合?** 改造项目→`/retrofit`，需求定义→`/requirements`

## 运行模式

```bash
/onboard              → 智能检测（询问读或写）
/onboard --read       → 只读取现有文档，不修改
/onboard --update     → 强制重新扫描更新
/onboard --realign    → 规范升级回扫：00-context.md 按当前 `context.v1` 查漏补缺（不重扫项目）。复用 [共享契约](../pipeline/references/realign.md)；推荐用户入口 `/pipeline realign`
```
...
3. 读取代码库盘点 + 采集运行态信息
   │
   ├── 委托 codebase-insight 获取结构化信息（模块、接口、数据对象）
...
> 结构化信息（模块、接口、数据对象）从 `docs/codebase-insight.md` 读取，不存在时委托 `codebase-insight` 生成。
...
### 交接方（工具 A）
1. 完成阶段性工作后，运行 `/onboard --update`
2. 将 `00-context.md` 内容传递给新 AI 工具

### 接手方（工具 B）
1. 运行 `/onboard --read` 了解项目
2. 从"待办任务"部分继续开发
3. 完成工作后运行 `/onboard --update` 更新上下文
