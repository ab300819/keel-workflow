...
**验证方式两档**：`可执行`（默认，优先——命令 / 脚本函数 / 基准 / 既有测试编号）与 `人工`（例外，须写明为何不可执行）。档位决定 `/ms-dev-workflow` 的执行路径，详见 [dev-workflow execution-flow.md](../ms-dev-workflow/references/execution-flow.md)。
...
- 通过追溯矩阵表达关联关系
- 编号一旦分配不可复用
- ⛔ `M-XXX` **不进追溯矩阵**（§5 各表不出现 M）。它是逻辑组织标识，只用来把多个 F 分成可交付的段；登记与消费边界见 [`M` 标识登记](../_shared/constraints.md)
...
- [ ] 「验收状态」列初值一律 `⏳ 待验证`。⛔ ms-requirements 不写 `✅ 已验证`——那是人实际用过之后由 `ms-dev-workflow` 在段末停点写入的事实，⛔ 不得由需求阶段预设
