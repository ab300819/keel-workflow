...
> 逐消费方的 legacy 行为见各自文件：[verify D1/D4](../../ms-verify/SKILL.md)、[verify impl B1](../../ms-verify/SKILL.md)、[test-cases](../../ms-test-cases/SKILL.md)、[sync trace/audit](../../ms-sync/references/)。
...
> - legacy 子分支：存量文档归属列全 `—` 或整列缺失时，`ms-dev-workflow` **不分段**，行为与 v2 完全一致；⛔ 不得因此跳过其他检查。
...
- 底层子动作（**编排实现细节，⛔ 不作为用户面命令暴露**，见 [constraints.md](../../_shared/constraints.md) `realign/no-direct-user-call`）：`--realign[=scope]`，scope ∈ {features / stories / ac / trace / **con** / **milestone** / needs-review}
