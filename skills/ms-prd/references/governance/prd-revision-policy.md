...
> 本 spec 把单条 FR/NFR、chunk、模板三类变更边界收纳到同一张治理表。目标是显性化已有机制，不把 PRD 流程扩展成 DevDocs 6 阶段治理框架。
>
> ℹ️ `docs/prd/` 是单需求一次性脚手架，无跨需求生命周期 / supersede / archive（多需求并行已移除，详见 [ms-prd 单需求脚手架原则](../../SKILL.md#单需求脚手架原则)）。需求收尾清理见 ms-prd `clear`。
...
| 机制 | 证据 | 状态 |
|---|---|---|
| 单 FR revise | `skills/ms-prd/SKILL.md` --revise 模式：定位 FR，保持 id 不变，原地覆盖，有 DevDocs 映射时置 `mapping_status: outdated` | [现状] |
| PRD 更新与 chunk 指纹 | `skills/ms-prd/SKILL.md` § 编排流程（PRD 更新）、`skills/ms-prd-parser/SKILL.md` § 变更检测：document/source fingerprint 对比后标记 unchanged/outdated/pending/removed | [现状] |
| chunk 状态机 | `skills/ms-prd-parser/SKILL.md` § Status 流转：pending、clarified、outdated、removed 的转换和保持原 chunk ID | [现状] |
| 模板 spec_version | `skills/ms-prd-parser/SKILL.md` § Chunk 文件、`skills/ms-prd-brainstorm/SKILL.md` § 输出文件 要求写入 `generated_by/spec_version/generated_at` | [现状] |
| realign 规则 | `skills/ms-prd-parser/references/realign.md:5-28`、`skills/ms-prd-brainstorm/references/realign.md:5-29`、`skills/ms-pipeline/references/realign.md:65-75` | [现状] |
...
|---|---|---|---|---|
| 单条 FR/NFR 内容 | 用户对 `FR-XX` 或 `NFR-XX` 提出修订 | [现状] 原地 revise，保持编号不变；如已有 DevDocs 映射则置 `mapping_status: outdated` | DevDocs 对应 F/FEAT 需要重新导入或人工确认 | ⚠️ 修改需求语义时必须确认；编号不重分配 |
...
1. [现状] `/ms-prd --revise FR-XX` 保持 `FR-XX` 编号不变并原地覆盖内容（`skills/ms-prd/SKILL.md` § --revise 模式）。
2. [现状] 若该 FR 已映射到 DevDocs，则 mapping 表追加或更新为 `mapping_status: outdated`。
3. [现状] `/ms-requirements --from-prd` 检测 `outdated` 条目后，原地更新对应 F/US/AC，不创建新编号（`skills/ms-requirements/SKILL.md` § --from-prd）。
...
3. [现状] `outdated/pending` 块进入 brainstorm；`removed` 块保留文件但标记状态，不物理删除（`skills/ms-prd-parser/SKILL.md` § 变更追踪约束）。
...
- [prd-parser realign](../../../ms-prd-parser/references/realign.md)
- [prd-brainstorm realign](../../../ms-prd-brainstorm/references/realign.md)
- [pipeline realign](../../../ms-pipeline/references/realign.md)
