...
/ms-feature F-XX realign           → 只对齐 F-XX 关联产物（调度 system-design/test-cases/dev-tasks/dev-workflow --realign=F-XX）；共享契约见 [../ms-pipeline/references/realign.md](../ms-pipeline/references/realign.md)
```
...
- 任务量小到可直接追加，无需调用 ms-dev-tasks；超出即改走标准档
