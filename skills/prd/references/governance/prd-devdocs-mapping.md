...
| 状态 | 语义 | 触发条件 | 写入方 | 下一步 |
|---|---|---|---|---|
| `active` | PRD FR/NFR 与 DevDocs F/FEAT 当前被认为一致 | 首次 `/requirements --from-prd` 成功导入 | [现状] requirements | 无需动作 |
| `outdated` | PRD FR/NFR 已修改，但对应 DevDocs 尚未更新 | `/prd --revise` 修改已映射 FR/NFR | [现状] prd | 运行 `/requirements --from-prd` 或人工处理 |
| `remapped` | 已重新导入，DevDocs 对应内容已更新；旧行保留审计 | `/requirements --from-prd` 处理 outdated 或重新导入 | [现状] requirements | 以最后一行为 authoritative row |
| `removed` | 对应 DevDocs F/FEAT 已被移除或废弃 | `/sync --back-propagate-prd` 检测到映射的 F 已不在 DevDocs 需求文件 | [现状] sync | 人工判断 PRD 是否保留或修订 |
...
| 场景 | 自动完成 | 必须人工 |
|---|---|---|
| PRD 修订后 DevDocs 过期 | [现状] `/prd --revise` 把 mapping 置为 `outdated` | 决定是否立即运行 `/requirements --from-prd`；确认需求语义是否仍成立 |
| `--from-prd` 重新导入 | [现状] 处理 outdated 时原地更新对应 F/US/AC，不创建新编号；追加 `remapped` 历史行 | 判断复杂拆分/合并场景是否需要新增 DevDocs 功能而非原地更新 |
| DevDocs 删除 F | [现状] `/sync --back-propagate-prd` 标记 `mapping_status: removed` | 判断 FR 是否也应废弃、改写、迁移到新版 PRD 或保留为产品历史 |
| DevDocs 新增 F 无 PRD 来源 | [现状] `/sync --back-propagate-prd` 标注“DevDocs 侧新增，无 prd 来源” | 是否补充 PRD、接受 DevDocs-only 需求、或启动 PRD 修订 |
