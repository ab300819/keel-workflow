...
| 机制 | 证据 | 状态 |
|---|---|---|
| 单 FR revise | `skills/prd/SKILL.md` --revise 模式：定位 FR，保持 id 不变，原地覆盖，有 DevDocs 映射时置 `mapping_status: outdated` | [现状] |
...
1. [现状] `/ms-prd --revise FR-XX` 保持 `FR-XX` 编号不变并原地覆盖内容（`skills/prd/SKILL.md` § --revise 模式）。
2. [现状] 若该 FR 已映射到 DevDocs，则 mapping 表追加或更新为 `mapping_status: outdated`。
3. [现状] `/ms-requirements --from-prd` 检测 `outdated` 条目后，原地更新对应 F/US/AC，不创建新编号（`skills/requirements/SKILL.md` § --from-prd）。
