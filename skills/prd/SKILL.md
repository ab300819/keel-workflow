---
name: prd
description: 将模糊产品想法或大型 PRD 整理为可供 keel 消费的需求，选择探索或解析路径。已明确需求编码用 requirements。
metadata:
  patterns: [pipeline]
...
编排产品需求处理流程，自动路由到 brainstorm 或 PRD 解析，产出结构化需求包供 keel 消费。
...
- PRD ↔ keel Mapping：[references/governance/prd-devdocs-mapping.md](references/governance/prd-devdocs-mapping.md)
...
**核心价值**：弥补 keel 对模糊想法和大型 PRD 的处理不足，提供需求探索到结构化输出的完整路径。
...
| keel 检测 | 检测 `docs/devdocs/01-requirements.md` | 存在 → 提取已有 F/US 列表作为补充上下文；不存在 → 跳过 |
...
4. 用 brainstorm 产出**原地覆盖** source_fr_path（保持 id: FR-03）+ 更新 requirements/index.md
    |
    v
5. 如果 FR-03 已有 keel 映射 → 将 mapping_status 置为 outdated
...
2. **影响分析**（"已安全吸收"判定，三条件全满足才算可清理）：
   ...
   - **取 authoritative row**：对每个 `product_id`，取 `requirements/index.md` 的 `## keel 映射` 表中**最后一行**为权威状态（不能因历史行是 active 就忽略后续 outdated/removed，见 [prd-mapping-status.md](references/prd-mapping-status.md)）；
   - **校验 keel 实存**：权威行 `devdocs_id` 必须在 `docs/devdocs/01-requirements.md` 当前 F 列表中实际存在；
   - 仅当「最新状态 ∈ {active, remapped} **且** 目标 F 在 keel 存在」→ 标记**可安全清理**；其余（无映射 / outdated / removed / 目标缺失）→ ⚠️ **孤儿 / 未同步高亮**，默认不删。
...
| 场景 | 输入 | 步骤 | 输出 / 门控 |
|---|---|---|---|
| 头脑风暴（idea → requirements） | 用户输入（一句话/简短想法） | Task: prd-brainstorm（完整模式），按 5W1H → 用户旅程 → MoSCoW 收敛 | 生成 requirements/ 小文档 + index.md；成熟度评估 → keel 衔接建议 |
...
未满足时在 index.md 标注缺失项，用户可选择仍然进入 keel。
...
- 需求文件**不分配 F/US/AC 编号**，编号权属于 keel
- FR-XX/NFR-XX 为 product 阶段唯一标识，进入 keel 后映射为 F-XXX
- 需求文件存放于 `docs/prd/`，不写入 `docs/devdocs/`
- 以小文档形式传入 keel：index.md 提供全局视图，各 FR-XX 对应独立功能领域
...
|   +-- index.md                <- 本需求总纲（含跨块合成结果 + keel 映射表）
|   +-- FR-XX-<topic>.md
|   +-- NFR-XX-<topic>.md
...
`docs/prd/requirements/index.md` 是当前需求以下信息的唯一权威源：需求清单、跨块合成结果（术语/假设/横切 NFR/冲突）、假设挑战、keel 映射。各 FR-XX/NFR-XX 分文件为对应需求项的内容权威。

- FR/NFR 在当前需求内续编、两位数字（`FR-XX`），一旦分配在本需求内不复用。
- 新需求从 `FR-01` 重起 —— 产品阶段编号是映射进 keel 后即作废的临时标识，无需跨需求全局唯一（依赖[新建需求门禁](#新建需求门禁强约束)确保旧脚手架已清理）。
...
| keel 衔接 | ready 时推荐 requirements --from-prd |
...
- `draft` → 提示开放问题清单，建议补充后再衔接，或用户选择直接进入 keel
