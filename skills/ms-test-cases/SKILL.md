---
name: ms-test-cases
description: 依据 DevDocs 需求设计可追溯的 UT/IT/E2E 用例文档。编写测试代码用 testing-guide；运行套件用 ms-test-run。
...
**最常见用法**: `/ms-test-cases`（自动检测初始/增量）

**不适合?** 跑测试→`/ms-test-run`，开发指导→`/ms-dev-workflow`
...
> 变更来源由 `/ms-sync` 维护：⛔ **代码里不写任何 DevDocs 编号**，连接方向是**文档 → commit**（反向依赖）。
...
> 行 / 分支覆盖率数值以 [`/testing-guide` 核心阈值表](../testing-guide/SKILL.md#核心阈值表) 为权威，本节不镜像（agent 侧改指针；产物模板允许内联，见该表「镜像边界」）。下表只定义 ms-test-cases 私有的**覆盖单位**要求。
...
AC 质量评估标准详见 [../ms-requirements/references/ac-quality-rubric.md](../ms-requirements/references/ac-quality-rubric.md)，验证 AC→测试覆盖时参照。
...
在呈现给用户确认前，加载 [../ms-requirements/references/ac-quality-rubric.md](../ms-requirements/references/ac-quality-rubric.md) 并自动验证：
...
| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 需求输入 | `/ms-requirements` | 前置：提供 F/US/AC 作为测试设计依据 |
| 新功能测试 | `/ms-feature` | 被调用：新功能需要新增测试用例 |
| Bug 回归测试 | `/ms-bugfix` | 被调用：Bug 修复需要补充回归测试 |
| 改进测试 | `/ms-insights` | 被调用：改进建议可能需要测试覆盖 |
| 追溯更新 | `/ms-sync` | 协作：trace 模式更新追溯矩阵代码位置 |
| 测试质量 | `/testing-guide` | 协作：编写测试代码时的质量约束 |
| 任务拆分 | `/ms-dev-tasks` | 后续：测试用例转化为开发任务 |
