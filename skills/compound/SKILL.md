...
**一句话**: 从已完成的开发周期中提取模式和经验教训。

**最常见用法**: `/compound`（完整提取）

**不适合?** 同步文档→`/sync`，项目交接→`/onboard`
...
```
sync：同步文档状态、追溯矩阵          → 文档层面的完成
compound：提取经验、沉淀模式          → 知识层面的复利
```

**互补关系**：sync 确保"这次做对了"，本 Skill 确保"下次也能做对"。
...
```bash
/compound                    → 完整流程（提取 + 写入 + 更新）
/compound --extract-only     → 仅提取，不写入文件（预览模式）
/compound --check            → 系统学习检查（仅评估是否有未沉淀的经验）
```
...
| 验证盲区 | verify 未检出但后续暴露的缺陷 | "verify --impl 未发现跨模块副作用" |
...
每次执行 `/compound` 时，额外检查验证流程的有效性——形成 **verify → 使用 → 发现遗漏 → compound 沉淀 → verify 改进** 的闭环。
...
> 此文件由 verify 启动时读取，作为额外检查项补充。形成持续改进的评估者调优闭环（[参考](https://www.anthropic.com/engineering/harness-design-long-running-apps)：评估提示需要多轮迭代调优）。

## 系统学习检查

每次执行 `/compound` 时，必须回答以下检查问题：
...
4. **验证盲区检查**：本次是否发现了 verify 未能检出的问题？
...
- [ ] **模式描述必须具体可操作**——写不出「换个项目照着做能落地」的，是总结不是模式
...
| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 前置 | `/sync` | 在 sync 完成后执行 compound |
| 前置 | `/verify` | 读取验证报告提取改进模式 |
| 前置 | `/dev-workflow` | 在工作流末尾推荐执行（批量模式默认） |
