...
> ⚠️ 术语消歧：本文 P 级 = verify **问题严重度**；与 test-cases / dev-tasks / insights 的 P0-P2 **优先级**（用例/任务/洞察排序）同名不同义，互不映射。

## P1 判定标准（阻塞发布，必须修复）

- **--docs**：原始需求遗漏（层 1）、AC 无设计支撑（层 2）、AC 无对应测试（层 3）
- **--impl**：AC 未满足（❌）、接口签名与设计不符、模块职责严重偏离、核心行为型 AC 与当前代码行为矛盾或无独立行为证据（定义见 [SKILL.md B3](../SKILL.md)）
...
## P2 判定标准（应修复，建议转入 insights）

- **--docs**：原始需求偏移（层 1）、AC 缺异常路径测试（层 3）
- **--impl**：AC 部分满足（⚠️）、数据流轻微偏离、非核心 AC 无法形成可复核证据链
...
| 优先级 | 含义 | 处理方式 |
|--------|------|----------|
| **P1** | 阻塞发布 | 必须修复，建议转入 dev-tasks |
| **P2** | 应修复 | 建议修复，建议转入 insights |
| **P3** | 可优化 | 可选修复，记录备忘 |
