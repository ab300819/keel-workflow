...
自描述**留在代码里**，keel 追溯**不留在代码里**。判据是：**代码丢了这条信息，还能不能被正确解读和维护。**

| | 自描述（INPUT/OUTPUT/POS） | keel 追溯 |
|---|---|---|
| 回答什么 | 这个模块做什么、依赖谁 | 这段代码为哪条需求写的 |
| 对没有 keel 上下文的维护者 | **有用**——不需要任何外部系统就能读懂 | **是噪音**——不知道 `AC-001` 是什么、去哪查、还在不在 |
| keel 丢失后 | 照常有效 | 变成指向虚空的指针 |
