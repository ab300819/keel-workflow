...
`/retrofit --realign`   → 规范升级回扫：00-baseline.md 按当前 baseline.v1 查漏补缺（⛔ 只补结构，不改带来源标注的正文）。详见 [references/realign.md](references/realign.md)；推荐用户入口 /pipeline realign

**不适合?** 新项目→`/pipeline init`，加功能→`/feature`
...
1. docs/devdocs/ 是否存在？
   ...
   └── 存在 → 有 00-baseline.md 且无 01~04？
       │
       ├── 是 → 基线已建（第三终态）→ 无需改造，提示下一步 /feature
       │
       └── 否 → 规范符合性检查
           │
           ├── 符合当前规范 → 无需改造
...
于是版本迁移流程**试图把刚建好的基线项目迁回旧的编号形态**——用户第二次跑 `/retrofit`
...
**存量代码不编号。** `01-requirements.md` 由首次 `/feature` 创建，`F-001` 从新需求起。
改造完成到首个需求之间，`/board`、`/test-cases`、`/verify` 会拒绝运行——**这是正确行为**，还没有需求，当然不能评审需求。
...
**关键规则**：基线不是终点。有新需求 → `/feature`；补充项目背景或把 `推导待确认` 升为 `用户确认` → `/requirements --context`（写入 `00-baseline.md`）。
...
| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 代码盘点 | `/codebase-insight` | 建立基线 Step 1 委托，产出清单层供提问出候选 |
| 基线增量补全 | `/requirements --context` | 把 `未知` / `推导待确认` 升为 `用户确认` |
| 需求审查 | `/requirements` | 审查需求文档（基线项目须先有 `01`，由首次 `/feature` 创建） |
| 设计审查 | `/system-design` | 审查系统设计（同上，须先有需求文档） |
| 测试用例 | `/test-cases` | 完善测试用例设计（同上，须先有需求文档） |
| 新增功能 | `/feature` | 改造后添加新功能 |
| Bug 修复 | `/bugfix` | 改造后修复问题 |
...
### 阶段边界约束（最高优先级）
- [ ] **⛔ 禁止继续：文档阶段不得产出实现代码**（恢复方式：使用 /dev-workflow 执行编码）
...
附带（由委托的 skill 各自产出，不由本 skill 写）：`docs/codebase-insight.md`（`codebase-insight`）、`AGENTS.md` 的领域术语与架构决策节（`agent-memory`）。
