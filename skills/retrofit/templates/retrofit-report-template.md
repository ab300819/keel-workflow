...
| 场景 | 必须执行 | 说明 |
|------|----------|------|
| **补充背景信息** | `/ms-requirements --context` | **推荐**：补充代码中看不出的背景、约束、参考资料 |
| 首次改造 | `/ms-sync` | 检查追溯健康度 |
| 有待补充项 | `/ms-requirements` | 完善需求文档 |
| 开始开发 | `/ms-dev-tasks` → `/ms-dev-workflow` | 执行任务 |
| 添加功能 | `/ms-feature` | 增量开发 |
