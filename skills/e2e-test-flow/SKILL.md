---
name: e2e-test-flow
description: 按已有 Markdown/HTML 用例对运行中的系统执行黑盒接口与浏览器 E2E 实测。设计用例用 ms-test-cases；运行仓库测试套件用 ms-test-run。
...
> 源码位置取自握手 `workspace_context.code_roots`（`inline` 时为单项，`path` = 仓库根绝对路径）。本 skill 与 DevDocs 独立，仅借拓扑上下文定位源码，不读 `docs/devdocs/`。契约见 [_shared/constraints.md](../_shared/constraints.md) §3 `task/workspace-context`；未传入时按 `inline` 缺省。
