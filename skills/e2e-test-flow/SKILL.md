...
> 源码位置取自握手 `workspace_context.code_roots`（`inline` 时为单项 `.`，即仓库根）。本 skill 与 DevDocs 独立，仅借拓扑上下文定位源码，不读 `docs/devdocs/`。契约见 [_shared/constraints.md](../_shared/constraints.md) §3 `task/workspace-context`；未传入时按 `inline` 缺省。
