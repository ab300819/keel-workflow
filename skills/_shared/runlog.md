...
related:
  ...
  - skills/_shared/constraints.md § 2（yaml-summary-v1 信封）
---
...
编排层（`ms-pipeline` / `ms-feature` / `ms-bugfix` / `ms-dev-workflow` 等任何通过 Task tool 委托的 skill）在**收到子 Agent 的 yaml-summary 之后**追加一条。
...
> 本节**刻意不使用 `⛔` 标记**。runlog 按设计不设门禁（规则 1），没有任何「阻塞并等待恢复」的语义；用 `⛔` 表达「别这么做」正是稀释该标记的典型用法。这里只用加粗。
...
⚠️ **本仓（skill 规格库）自身不跑 DevDocs**，304 个提交中零个真 trailer。上述统计只能在**消费 DevDocs 的项目**里做；判决点的计数同理，在本仓不会推进。
