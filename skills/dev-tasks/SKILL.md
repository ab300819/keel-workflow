...
- 用户要求拆分开发任务
- 用户需要迭代/Sprint 规划
- 来自 `/feature`、`/bugfix`、`/insights` 的增量需求
...
**最常见用法**: `/dev-tasks`。想少确认几步直接说，不必记参数。

**不适合?** 执行任务→`/dev-workflow`，设计还没做→`/system-design`

## 运行模式

```bash
/dev-tasks                  → 标准模式（逐步确认）
/dev-tasks --realign[=scope] → 规范升级回扫（任务结构/字段差距补齐；不改任务状态）。详见 [references/realign.md](references/realign.md)；推荐 `/pipeline realign`
```
...
4. **定义依赖**：建立任务执行顺序
5. **评估范围**：确保任务粒度合适
6. **生成任务文档**：按层级分组输出，每个任务包含执行步骤参考（实际强度由 `review_profile` 决定，S1~S11 完整矩阵见 `dev-workflow` execution-flow）：
   - 🔴 核心逻辑（通常 audit/guarded）：红→绿→重构三步参考（`TDD 执行步骤` 区块）
   - 🟡 接口层（通常 guarded）：红→绿两步参考（`TDD 执行步骤（推荐）` 区块）
   - 🟢 UI 层（通常 fast/guarded）：实现→验证→补测试（普通执行步骤）
   - ⚪ 基础设施（通常 fast）：`TDD 模式 = ⚪ 不适用` → **S4/S5 退化**（不先写断言与红验），实现后由验证手段确认；⛔ S2/S3 与 S6/S7 不退化
...
根据任务类型分层。层级**仅作风险输入之一**——实际 `review_profile` 按下方[风险分类器](#review_profile-初始提议风险分类器)组合信号确定，执行强度由 `review_profile` 决定（S1~S11 完整矩阵见 dev-workflow execution-flow）：
...
- [ ] 任务编号格式：`T-XX`（顺序编号）；取当前最大值按 [`id/scan-word-boundary`](../shared/constraints.md)——⛔ 裸 `grep -o 'T-[0-9]+'` 会把 `UT-001` 里的 `T-001` 当成任务编号
- [ ] 后批次任务详细程度不低于首批次（TAR 完整性、路径具体性、粒度一致性）

### 需求追溯约束

- [ ] **每个任务必须关联 (`F-XXX` + `AC-XXX`) 或 `CON-XXX`**
- [ ] **编码类任务必须关联测试用例 (`UT/IT/E2E-XXX`)**，来自 `03-test-*.md`
- [ ] **非编码类任务**（只关联 `CON`）不强制测试编号，但「测试方法」必须给出**可复核的通过 / 失败判据**（⛔ 不接受「检查配置正确」这类无判据表述），并标 `TDD 模式 = ⚪ 不适用`
- [ ] **TDD 执行步骤引用 AC 编号与测试编号的要求，仅适用 `TDD 模式 ≠ 不适用` 的任务**
- [ ] ⛔ **有用户可观测行为的任务不得用 `CON` 绕过 `AC`**。判不准时默认走 `F+AC`

**什么是非编码类任务**：工作内容是改配置、加资源、调元数据、改文案、删权限声明——**没有可写测试的业务逻辑**。上架合规、日志规约、安全基线、构建守卫都属于这类。它们关联 `01-requirements.md` §6 的 `CON-XXX`（约束编号），落 `04-dev-tasks-infra.md`。

> ⛔ **反模式**：用「缺了会导致 AC-001~AC-004 全部失败」这类间接论证给配置任务挂 `AC`。
> 那不是追溯，是修辞——它让追溯矩阵看起来满，实际指向的是别的需求。
...
| 来源 Skill | 触发场景 | 操作 |
|-----------|---------|------|
| `/feature` | 新增功能需求 | 追加任务到列表 |
| `/bugfix` | Bug 修复需求 | 插入高优先级任务 |
| `/insights` | 改进建议确认 | 追加任务到列表 |
...
3. 建议从第一个任务开始，或使用批量模式：`/dev-workflow T-01~T-XX`
4. **执行任务时必须使用 `/dev-workflow`**
5. 支持按功能点（`F-XXX`）或用户故事（`US-XXX`）批量执行

> **重要**：直接写代码而不使用 dev-workflow 会导致追溯矩阵的「变更来源」列不被写入，
> 使 `/sync` 无法自动追溯，破坏文档↔代码的闭环。
...
```bash
/dev-tasks --backfill-design → 回填 design_ref（由 pipeline design 委托）
```
