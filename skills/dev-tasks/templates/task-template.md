...
> **Frontmatter 必填**（字段语义权威见 `shared/constraints.md` `realign/template-frontmatter-minimum`）：正文最顶部插入以下 YAML，之后才是 `# 开发任务：...` 标题。

```yaml
---
generated_by: dev-tasks
...
> 层级为风险输入，执行强度由 `review_profile` 决定；S1~S11 完整矩阵见 dev-workflow execution-flow。
...
| 消费方 | `/dev-workflow` S1 读到即 ⛔ 停（见其「失效任务卡前置门」）|
