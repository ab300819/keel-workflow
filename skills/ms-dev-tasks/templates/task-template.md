...
| **审查状态** | `review_state` / `batch` / `due` / `pending_reason` / `ext_review_state`（按需填，见 [dev-workflow §任务台账](../../ms-dev-workflow/SKILL.md)）|
...
| 规则 | 内容 |
|---|---|
| 写入时机 | ADR 推翻既有决策时，由「已知受影响制品」表逐条回填（见 [system-design incremental-design.md](../../ms-system-design/references/incremental-design.md)）|
