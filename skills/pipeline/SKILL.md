...
- **工作区模式探测（`init`）**：`ms-requirements` 完成后（首次产生 `docs/devdocs/`）、`agent-memory --update` 之前，执行一次工作区模式探测，见 [layout/workspace-shell.md § 探测](references/layout/workspace-shell.md#探测)。探测结果通过 `devdocs_frontmatter` 输入（`workspace_mode` / `code_roots`）随同一步 `Task: /agent-memory --update` 一并传入，写入 `AGENTS.md` 的 `devdocs:` frontmatter，字段契约见 [agent-memory/SKILL.md](../agent-memory/SKILL.md) § devdocs frontmatter 写入(可选)。无 `.gitmodules` 时静默判定 `inline`，不打扰用户。retrofit 场景的探测路径由 [retrofit/SKILL.md](../retrofit/SKILL.md) 自己承载（retrofit 非 pipeline 管辖，见下方约束"识别到 retrofit 场景时，路由到 `/ms-retrofit` 并退出 pipeline"）。
