...
- 用户主入口：`/pipeline realign`（由编排层自动调度到此子流程）
- 底层直用（调试/精细场景）：
  - `/dev-workflow T-03 --realign` — 单任务 realign
  - `/dev-workflow F-01 --realign` — 按功能点过滤
  - `/dev-workflow --all --realign` — 全部已完成任务 realign（需工作区洁净）
