...
**一句话**: 执行开发任务，采用骨架先行 + 分层 TDD。

**最常见用法**: `/ms-dev-workflow T-03`（单任务）、`/ms-dev-workflow T-01~T-05`（批量）

**不适合?** 拆分任务→`/ms-dev-tasks`，修 Bug→`/ms-bugfix`
...
> **探索模式最小行为验证**：
> 探索模式可跳过文档强制（AC/追溯标注/完整 TDD）和完整对抗式验证，但**不得跳过**：
> - S6 绿验（skipped/todo=0 仍然生效）
> - 至少 1 条"目标行为证据"（AC 或 UT/IT 断言 / `--ui --live` 截图 / 显式豁免+原因）
> - 写入 Commit 1 `Exploration-Mode: true` 尾注，便于事后 `/ms-retrofit` 补 AC。
> 若无任何行为证据，探索模式仍 ⛔ 阻止提交（恢复方式：补最少 1 条证据或登记豁免原因）。
...
4. 完成检查
   ├── 基础检查（始终执行）
   │   ├── AC 完备性表：逐条 AC 产出「证据类型/代码或测试位置/判定」
   │   │   └── 任一 AC 缺证据 → ⛔ 禁止继续（补实现/补证据后重跑）
   ...
   │   ├── 🟡：/ms-verify --impl（默认必做）
   │   ├── 🟢：/ms-verify --ui --impl（有设计稿） 或 Phase 2-UI 自查 + --impl（无设计稿）
...
| UI 对齐 | `/ms-verify --ui` | **按层级默认必做**：🟢 有设计稿时自动触发；无设计稿降级为 Phase 2-UI 自查 |
...
- [ ] UI 层任务标记 🟢 可选 TDD；同上条件升级规则，且清单 Blocker 项必须对应证据（见 ui-quality-checklist.md）
...
- [ ] **S8 必须产出 AC 完备性表**（逐条 AC：编号/证据类型/代码或测试位置/判定）
- [ ] **任一 AC 缺失有效证据 → ⛔ 禁止继续**（恢复方式：补实现或补测试后重新生成证据表）
...
**AC 完备性表模板**（S8 必须输出，作为 Commit 1 附加信息或决策日志的一部分）：

```markdown
| AC 编号 | 证据类型 | 位置 | 判定 |
|---------|---------|------|------|
| AC-001 | UT 断言 | tests/user.spec.ts:42 `it('拒绝空密码')` | ✅ 满足 |
| AC-002 | 实现代码 + IT 断言 | src/auth.ts:88, tests/auth.int.ts:15 | ✅ 满足 |
| AC-003 | UI 清单 + --ui --live 截图 | ui-checklist §2-UI.2, artifacts/T-03-disabled.png | ✅ 满足 |
```

证据类型白名单：`UT 断言` / `IT 断言` / `E2E 断言` / `实现代码` / `UI 清单` / `--ui --live 截图` / `--ui --live trace` / `显式豁免（附原因）`。任何不在白名单内的证据必须升级为显式豁免。
...
| 🟢 UI 层 | 有设计稿 → `/ms-verify --ui --impl`；无设计稿 → Phase 2-UI 自查 + `/ms-verify --impl`（降级） | 不触发 | `--review` 叠加完整对抗式验证 |
...
- [ ] **"已完成"任务必须通过证据复核才允许跳过**（AC 表可复核 + 测试无 skip/todo + trace 已同步；任一缺失 → 复核续做，不直接跳过）
- [ ] **Git 历史有 code+doc commit 但任务状态≠已完成**：不再直接跳过，改为进入证据复核路径
- [ ] 旧任务（Fix 4 前完成，无 AC 表）→ AskUserQuestion：复核续做 / 登记豁免原因 / 终止
