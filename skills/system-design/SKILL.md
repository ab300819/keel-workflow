...
2. 询问偏好 → 技术栈、平台、集成需求
      │
      ▼
3. 探索代码 → 了解现有架构，针对需求涉及的接口/实体/表确认已有实现
      │
      ▼
3.5 外部研究（按需）→ 详见"设计前外部研究"章节
      │
      ▼
4. 呈现设计草案 + 对抗性审查（AskUserQuestion）
      │  先生成草案，再从三个视角自动生成挑战并回应
      │  （详见 references/design-review.md），一并呈现给用户确认
      ▼
5. 用户确认方案 → 确认方向或调整
      │
      ▼
6. 生成 docs/devdocs/02-system-design.md 文档（含对抗性审查结论 + 关键选型 ADR）
      │
      ▼
...
5. 呈现变更方案 + 对抗性审查（AskUserQuestion）
      │  先生成变更方案，再从三个视角审查变更影响并回应
      │  聚焦：用户价值/架构兼容性/可测试性，一并呈现给用户确认
      ▼
6. 用户确认方案 → 确认方案或调整
      │
      ▼
...
| 文件 | 包含章节 |
|------|----------|
| 02-system-design.md | 1-7, 15-16: 平台、架构、技术选型、模块、接口签名、模式、代码结构、设计审查、ADR |
