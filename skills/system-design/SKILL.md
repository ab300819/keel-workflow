...
- **快速档**：L1~L4 合并为一次 AskUserQuestion 展示，但**"越界反例自查 + L1~L4 边界校验 + 原则校验表 + 总确认"不得跳过**
...
| 模式 | 确认时机 | 方案内容 |
|------|----------|-----------|
| 初始设计 | L1~L4 四层分层推进（或快速档合并展示）后 | 技术选型、架构、L2 模块、L3 接口、L4 数据、原则校验 |
| 增量设计 | 影响分析 + 兼容性评估后 | 影响范围、变更策略、**修订清单**、原则校验 |

**约束**：
- 用户确认前：只展示方案，不写入任何文件
- 用户确认后：仅生成 `docs/devdocs/` 下的文档，严禁编写实现代码
- 快速档：跳过分层交互，但保留越界反例自查 + L1~L4 边界校验 + 原则校验表 + 一次性总确认
