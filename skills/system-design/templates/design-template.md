...
> **Frontmatter 必填**（字段语义权威见 `shared/constraints.md` `realign/template-frontmatter-minimum`）：正文最顶部插入以下 YAML，之后才是 `# 系统设计：...` 标题。
...
> **实现类不属于 §5**：实现细节在 `/dev-workflow` 阶段产出，设计文档不定义实现类名。
>
...
> ADR 编号在项目内全局递增，不重复（取最大值按 [`id/scan-word-boundary`](../../shared/constraints.md)：整词匹配 + 数值序）。状态可选：`已采纳` | `已废弃` | `已取代`。
