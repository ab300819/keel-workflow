---
name: prd-brainstorm
description: 探索模糊产品想法或澄清 PRD 片段，形成结构化需求。拆分大型 PRD 用 prd-parser。
...
**一句话**: 通过 5W1H/MoSCoW 等框架，将模糊想法结构化为需求。

**最常见用法**: `/prd`（推荐入口，自动调度 brainstorm）
...
- 用户想要探索和澄清需求
- 来自 `prd` 的块澄清委托
- 来自 `prd-parser` 的分段需要深入探索

## 运行模式

```bash
/prd-brainstorm                → 完整头脑风暴模式（一句话想法）
/prd-brainstorm --chunk <path> → 块澄清模式（PRD 分段逐块澄清）
/prd-brainstorm --realign      → 规范升级回扫：FR/NFR 结构差距补齐（不新增/修改业务内容）。复用 [共享契约](../pipeline/references/realign.md)；推荐 `/pipeline realign`
```
...
5. 功能发散 → 收敛
   │
   ├── 发散：列出所有可能功能点
   ├── 收敛前审视：当前功能列表是否在解决正确的问题？（轻提示，正式记录由 prd 写入 index.md）
   ├── 收敛：MoSCoW 分类
...
**Frontmatter 必填**：生成文件时必须在 frontmatter 顶部包含以下 3 个 realign 元数据字段（与 A 类模板同级约束；缺字段会被 `/verify --schema-drift` 判为 `legacy`）：
- `generated_by: prd-brainstorm`
- `spec_version: fr.v1`
