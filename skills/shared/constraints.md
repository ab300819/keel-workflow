...
related:
  ...
  - skills/shared/runlog.md（运行日志：形状、纪律、判决点）
...
| rule_id | 字段 | 定义 |
|---|---|---|
| `yaml-summary/skill-name` | `skill` | 返回当前 skill 的 `name`，例如 `requirements`。 |
...
- `confirm/scope-inflation` [新增]：**改动面超出用户诉求一个数量级时，必须停下来确认，⛔ 不得先做完再汇报。**
  ...
  | 信号 | 例 |
  |---|---|
  | 用户点名改 N 处，实际要改 ≥10N 处 | 用户说「改 `dev-tasks` 一条硬约束」，实际在改 36 个文件 |
...
- `realign/single-entry`：所有文档体系升级与健康度审查动作统一通过 `/pipeline realign [--scope=<spec|prd-mapping|health>]` 入口，不再以散落 flag 形式暴露给用户。
...
- `realign/no-direct-user-call`：各 skill 内部 `--realign` 子动作（如 `/system-design --realign=layer3-only`）属编排实现细节，不作为用户面命令暴露；用户面只承认 `/pipeline realign` 入口。
...
| 消费方 | 对 `CON` 的行为 |
|---|---|
| `dev-workflow` | 任务标 `TDD 模式 = ⚪ 不适用` 时按该档执行（**字段本已存在**），⛔ 不解析 CON 的类别与语义，只执行任务卡的「测试方法 / 验收标准」 |
| `verify` / `sync` / `test-run` / `test-cases` / `board` / `system-design` / `health-lint` 计分 | ⛔ **既不拦也不报**。`CON` 不进它们的覆盖率、孤立判据、健康分、覆盖门 |
...
| owner | `requirements`（划分）+ `dev-workflow`（分段执行与停点） |
...
| 消费方 | 对 `M` 的行为 |
|---|---|
| `verify --readiness` | **仅**检出「跨 M 反向依赖」（M1 的任务依赖 M2 的任务）并提示重划——那是划分错误，⛔ 不自动兜底。其余维度不看 M |
| `sync` | 归档时 **M 未验收通过的 F ⛔ 不归档**（否则唯一成员源先于验收消失）；⛔ 不维护 M 的状态，人验事实住在检查点 |
| `backlog` | 接受 `M-XXX` 作 `source_id`，承接该段冒出、尚无任何编号的新需求 |
| `test-run` / `test-cases` / `board` / `system-design` / `health-lint` 计分 | ⛔ **既不拦也不报**。`M` 不进它们的覆盖率、孤立判据、健康分、覆盖门 |
...
⛔ **不得把「用什么工具验证」当作分类依据。** `git diff` / PR 评审 / `grep` 只是执行手段；对象是行为需求就走 `F+AC`，是非编码需求才走 `CON`（`dev-tasks` SKILL.md:187~191 的既有边界）。⛔ 也不得反过来因为"能自动化"就把一条人工判据升格成新测试类型——`tar-rubric.md` 的可测试判据原文即「可通过自动化**或手动**测试验证」。

> **为什么只要契约、不要禁令**：前缀的语义看名字猜不出来。本仓实测：`ai-code` 的 `R-1~R-4` 看着像风险清单，实为重定向安全判定规则（origin 严格相等 / 编译期白名单前缀 / 拒绝残留百分号编码，`02-system-design.md:395`）；`B-10` 看着像并行批次，实为"白名单放宽 + 回归范围"的登记条目（`:1467`）；`GAP-` / `BLK-` 是**当前闭环内**的阻塞事实，与 `backlog` 的"暂时不进入当前闭环"暂缓池语义不同。⇒ 一张"按前缀名归并到既有槽位"的映射表会逐条判错，把设计规则塞进 backlog。
>
...
>
> 消费方：产出编号的 skill（`requirements` / `system-design` / `dev-tasks` / `test-cases` / `bugfix` / `insights`）+ `health-lint` `id/unknown-prefix`。
...
>
> 消费方：`requirements`（编号续编，含 `M-XXX`）/ `dev-tasks` / `bugfix` / `test-cases`（编号续编）+ `agent-memory` devdocs-state 模板（推导命令）+ `health-lint` `id-reference-check` / `state/max-id-stale`（上界推导）。

## 差异点与跳过项

### 已标注语义差异点

- `diff/readonly-generated-output`：`codebase-insight` 是只读分析，但会写自身分析产物；`onboard` 同时有 `--read` 与 `--update`，不能统一成“永不写入”。
...
### review_profile 三档
| 档 | 双 Agent 红绿 | 质量地板 | 前置验证 `/verify --impl` | 独立审查(Phase 1~3 + Phase 4) |
|----|:---:|:---:|:---:|------|
...
- 清算入口:`/verify --review-drain`(批/sprint 边界集中跑延后审查)。
...
- `enum/no-extension`：消费方（verify / adversarial-review / dev-workflow 全部 references）不得扩展枚举值或私有化语义。
...
> 字段呈现与填写时机见 [dev-workflow SKILL.md §任务台账](../dev-workflow/SKILL.md)；消费方：`/verify --review-drain`（按 `batch`/`due` 清算）、断点续做 Step 1.5 证据复核。
>
