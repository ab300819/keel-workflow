---
name: prd-parser
description: 拆分大型 PRD 文档、PDF 或截图，保留原文并建立 FR/NFR 分类。需求澄清用 prd-brainstorm。
...
**最常见用法**: `/prd`（推荐入口，自动调度 parser）

**不适合?** 需求已明确→`/requirements`，想头脑风暴→`/prd`

**Realign**：`/prd-parser --realign` 按当前 `chunk.v1` 扫描 chunk frontmatter/分段字段差距补齐，不重新解析 PRD。共享契约见 [../pipeline/references/realign.md](../pipeline/references/realign.md)；推荐 `/pipeline realign`。
...
**Frontmatter 必填**：生成 chunk 时必须在 frontmatter 顶部包含以下 3 个 realign 元数据字段（与 A 类模板同级约束；缺字段会被 `/verify --schema-drift` 判为 `legacy`）：
- `generated_by: prd-parser`
...
- [ ] 拆分完成后必须展示摘要并等待用户确认
- [ ] **摘要必须逐块列出标题与行数**，不得只给总量——用户要据此判断分块边界是否合理

## Skill 协作

| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 编排调度 | `/prd` | 被调用：pipeline 委托 PRD 解析 |
| 块澄清 | `/prd-brainstorm` | 后续：chunks 传入 brainstorm 逐块澄清 |
| 分类终判 | `/prd-brainstorm` | 后续：brainstorm 可复判 FR/NFR 分类 |
...
| 完成场景 | 建议下一步 |
|----------|------------|
| 首次解析完成 | 返回 `/prd`，由 pipeline 逐块调用 brainstorm |
| 变更检测完成 | 返回 `/prd`，仅对 outdated/pending 块调用 brainstorm |
| 独立运行 | `/prd-brainstorm` 逐块澄清 |

> **提示**：本 skill 仅负责拆分和保留原文，需求解读和澄清由 `/prd-brainstorm` 负责。
