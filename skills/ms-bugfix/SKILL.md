...
> **Realign 模式**：已完成的 bug 修复记录（BUG-XX 条目、测试、修复代码）在 DevDocs 规范升级后按新 spec_version 查漏补缺。共享契约见 [../ms-pipeline/references/realign.md](../ms-pipeline/references/realign.md)。调度策略：按 BUG-XX 的 `关联` 字段追溯对应 F-XX 与 T-XX，委托 `/ms-dev-workflow <T-XX> --realign`。推荐用户入口 `/ms-pipeline realign`。
