...
各层级任务的详细执行流程。**所有模式（含单任务）均通过双 Agent 模型执行：Test Agent 写骨架+测试 → 编排器红色验证 → Impl Agent 写实现+重构 → 编排器完成检查+提交**。文档同步（/devdocs-sync + Commit 2）由编排器调度。
...
| S8 | 验证 AC 满足度 | 编排器 | `ac_verified` |
| S9 | 对抗式验证 | 编排器 | `review_done` |
...
│                               │                        │
│                               ▼                        │
│  13. 更新任务状态 + /devdocs-sync → Commit 2            │
│                                                        │
└────────────────────────────────────────────────────────┘
```
