---
name: code-quality
description: 编写或审查代码时提供 MTE、命名、注释、日志与可维护性指导。系统性重构执行用 refactor。
...
| 来源记录式/注释式追溯 | 记录"来自 UT-XX/AC-XX/后置条件/行为契约"等过程来源。⛔ **不得新增** `@satisfies` / `@requirement` / `@verifies` / `@testcase` 标注——指向外部系统的编号引用，对没有 keel 上下文的维护者是纯噪音，keel 丢失后更是指向虚空的指针；追溯走文档侧（反向依赖）。存量标注留着不清理，只约束新增 |
...
- **简单方案优先**：简单场景不套复杂模式
- **可选增强 · 专项复杂度审查**：环境若提供专查过度工程的审查能力（如 `/ponytail-review` 审 diff、`/ponytail-audit` 审全仓），可在本节自查后追加一轮——它只找「该删什么」，与本 skill 的正确性 + 阈值审查互补。**无该能力就止于本节清单**，⛔ 不因缺它跳过或降低最小审查清单；其结论按本 skill 反馈分级归一化后再落审查报告，⛔ 不另立一套级别
- 正反例见 [references/mte-examples.md](references/mte-examples.md)
