...
1. **基线增量补全**：`/retrofit` 建立 `00-baseline.md` 后，把当时标为 `未知` / `推导待确认` 的条目补齐
...
不为基线补全新建 `/retrofit --baseline-update` 一类的入口：补全基线是「补背景」，不是「改造项目」。 <!-- health-lint-disable-line flag/dangling-reference: 反例，非引用 -->

### 写入基线怎么做

前置：`docs/devdocs/00-baseline.md` 存在。
⛔ 此时**不得**创建 `01-requirements.md`——它由首次 `/feature` 创建。
