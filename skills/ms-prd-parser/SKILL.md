...
**Realign**：`/ms-prd-parser --realign` 按当前 `chunk.v1` 扫描 chunk frontmatter/分段字段差距补齐，不重新解析 PRD。共享契约见 [../ms-pipeline/references/realign.md](../ms-pipeline/references/realign.md)；推荐 `/ms-pipeline realign`。
