...
⛔ **不接受 `M-XXX` 指定符。** 里程碑是逻辑组织概念，不是执行单元（见 [`M` 标识登记](../../_shared/constraints.md)）。要跑某段就按它的 `F-XXX` 或 `T-XX` 范围指定。
...
⛔ **不自动兜底**。M 是人按需求规模划的产品切片，出现反向依赖说明切错了。由 `/ms-verify --readiness` 检出并要求用户重划（见 ms-verify D5）。执行期若仍检出，⚠️ 必须确认：调整划分 / 合并这两段 / 明确接受（本次不分段执行）。
...
**落点**：backlog `## Entries` 每条本就是自由文本块，摸查结果写正文（模板的 Context 段）。`source_id` = `M-XXX` 的条目同源第二条**默认追加不再询问**（见 ms-backlog 门控例外）。
...
⛔ **验收状态不进检查点。** 它的唯一持久源是 `01-requirements.md` §2.5 的「验收状态」列——检查点是批次内临时状态，把跨批次的人工验证事实放进去，批次一结束就没了权威性，`ms-sync` 归档判据也会落空。

⛔ `resume_command` **不生成 `M-XXX` 指定符**（M 不是执行单元），仍用 `T-xx~T-yy`。段级门由 Step 0 读 01 §2.5「验收状态」把守，不靠命令形状表达。
`normalized_intent` 已约定持久化在同一文件（见 [constraints.md](../../_shared/constraints.md) `task/normalized-intent-shape`），新增字段与之并列。
