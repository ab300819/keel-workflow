...
2. **dev-workflow embedded-headless 模式**（`/dev-workflow` S9 Phase 4）
...
> **适用范围限定**：以下三级链仅适用于 adversarial-review skill 的 **multi-turn 模式**。`dev-workflow` S9 Phase 4 embedded-headless 模式**只消费 T1/T2 子集，不执行 T3 分支**——T3 Task 子 Agent 是同进程独立上下文，不满足 dev-workflow "外部独立审查" 承诺。本文档"降级探测流程"和"T3"相关段落仅在 skill multi-turn 模式下生效。
...
> **为什么区分两行**：`uncommitted: true` 在改动已落盘时会得到**空 diff**——审查照常返回"无问题"，但它什么都没看到。本 skill 独立调用时默认审工作区（未提交），故用第一行；`dev-workflow` S9 Phase 4 的 **drain 侧**（Commit 1 已落盘后集中补审）必须用第二行。判定见 [verification-flow.md § diff 源](../../dev-workflow/references/verification-flow.md)。
