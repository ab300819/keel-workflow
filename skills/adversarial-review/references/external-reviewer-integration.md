...
> adversarial-review skill 对外部审查输出做二次加工（解析、映射、F-XXX 合成、收敛评分），是 skill 层面的有意架构选择。
...
| 审查类型 | 使用工具 | 说明 |
|----------|---------|------|
| 代码审查（diff / 文件） | `mcp__codex-mcp__review-code` | 传入 prompt（审查 brief）+ uncommitted: true |
