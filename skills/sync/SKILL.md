...
**最常见用法**: `/sync`（默认 trace+audit+check）、`/sync --archive`（归档历史 docs）、`/sync --back-propagate-prd`（反向同步 PRD mapping）

**不适合?** 验证对齐→`/verify`，提取经验→`/compound`
...
```bash
/sync                    → 默认：trace + audit + check，可带任务范围如 T-01 T-02
/sync --archive [requirements|design|tests|tasks] [--release v1.0.0]  → 归档历史 docs
/sync --back-propagate-prd  → 反向同步 prd 映射状态
```
...
- `check`：默认流程内置检查，不再暴露 `/sync --check`
- `absorb`：默认流程内置低风险吸收，不再暴露 `/sync --absorb`
- `schema-drift`：迁移到 `/verify --schema-drift`，sync 仅在 health report 合并 verify 产出的 drift 结果

### 模式对比

| 用户入口 | 检查 | 自动更新 | 用户确认 |
|----------|------|----------|----------|
| `/sync` | ✅ trace+audit+check | ✅ 低风险吸收/状态更新 | ✅ 高风险/写入前 |
| `/sync --archive` | ✅ 归档条件 | ✅ 归档文件 | ✅ 全部 |
| `/sync --back-propagate-prd` | ✅ PRD mapping 对比 | ✅ 映射状态 | ✅ 写入前 |
...
| 场景 | 协作 Skill | 说明 |
|------|-----------|------|
| 开发完成 | `/dev-workflow` | 被调用：任务完成后触发 trace 阶段 |
| 任务完成后 | `/dev-tasks` | 执行任务后触发同步 |
| 测试追溯 | `/test-cases` | 协作：更新追溯矩阵代码位置 |
| 需求变更 | `/feature` | 新功能添加后同步 |
| Bug 修复 | `/bugfix` | Bug 修复后更新文档 |
| 洞察确认 | `/insights` | 改进建议确认后同步 |
| 项目改造 | `/retrofit` | 改造后全量同步 |
...
| 代码已实现文档落后 | `/sync` | 状态未更新、新内容未登记，默认流程处理低风险吸收 |
...
```text
/sync            # trace → audit → check 自动串行 → 显示报告 → 确认更新
```
