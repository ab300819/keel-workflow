...
ms-verify `--impl` 维度的一组增强约束详细判定。SKILL.md「检查约束」节列出短规则条目，本文件展开判定逻辑。
...
**不得**把项目实战发现提拔为本 skill 的默认强制约束；理由与边界见 [`_shared/constraints.md`](../../_shared/constraints.md) § `doc/project-rule-boundary`。
