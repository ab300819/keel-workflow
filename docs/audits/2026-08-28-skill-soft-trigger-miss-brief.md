...
3. ~~放弃 `npx` 安装路径，只走 Plugin Marketplace~~ —— **✅ 2026-09-16 已触发**：
   用户决定改用插件安装方式，覆盖面问题消失。⇒ 本文「判定暂不做」的**前提已变**，
   hook 从"装不上"变为"可生效"。首批两个试验件见 [hooks/README.md](../../hooks/README.md)，
   ⚠️ 仍未在真实会话中跑过。
   遗留待验：模板里 72 处裸名 `/ms-xxx`（20 个文件）会复制进用户项目，
   **插件装完后裸名是否仍可解析未验证**；⛔ 不要在验证前批量改写。
