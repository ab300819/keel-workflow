...
| **代码污染** | `@satisfies` / `@verifies` 注释嵌入代码（636 处，**2026-05 对 mic-en 的统计，未复核**）| DevDocs 元信息侵入代码；DevDocs 丢失后它就是指向虚空的指针 | **反向依赖**：代码零编号，文档记 `<repository>@<sha>`（存量留着不清理，只约束新增）|
...
- 治理盲区收敛：scope=health（结构/索引/过大 3 维）+ health-lint rule（state/* + dead-link + adr-only-revision，清单以 [health-lint-implementation.md](skills/ms-pipeline/references/health-lint-implementation.md) Rule 集表为权威），layout.v1+v2 通用
...
- **分层记忆原则（已落地为 SSOT）**：决策/执行/数据三层分离，权威在 [_shared/constraints.md §9](skills/_shared/constraints.md)。**结构基础已存在**（编号文件提供三层 owner 位置），现役 `state/*`、`adr-only-revision` 只兜**部分**退化症状，**主要靠人工 review lens 承载**（非自动兜底）。
