...
**验证限制(2026-09-17)**:本机 ponytail 与 claude-mem 的 SessionStart hook 常驻,两条增强的验收测试都存在**混叠**——「条款让我做」与「hook 已让我知道」无法分离(两个独立会话各自主动指出这一点);且「无该能力则止于本节清单」分支从未被走过。要干净验证需在未装这两个插件的环境里跑。已验的是:条款被逐字引用触发、外部结论按 keel 分级归一化、⛔ 历史命中不替代证实三项均成立。
