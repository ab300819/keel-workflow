...
前几稿要求迁移前做完整备份。**那是历史改写方案的残留，历史改写砍掉后它不成立了**：

- 整仓 `mv` 进外壳仓——不销毁任何东西（实测：未推送提交、staged、untracked 全在）
- `git submodule add` 是登记不是 clone——不碰工作区

全流程唯一的删除动作是最后「从代码仓清掉 DevDocs 产物」，而它发生在文档已复制到外壳仓**之后**，删的又是 git 已跟踪的文件（历史里还在）。
...
⛔ 校验未通过不得执行删除。⛔ 不用 `reset --hard` 处理任何中间状态。
...
3. **§5.5 取消备份是否留有真实风险** —— 论据是「整仓 mv + 登记式挂载不销毁东西，唯一删除动作在复制校验之后」。请找出这个论据不成立的情形。
