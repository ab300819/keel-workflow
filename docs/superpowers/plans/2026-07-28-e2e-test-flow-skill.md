...
2. **`## 2. 写型用例执行规则`** —— ⚠️ **不得复述规则正文**(DRY,pre-flight 裁决):本节以一句话 + 链接指回 SKILL.md 的「写型用例三条规则」作为唯一权威,例如:
   ...
   > 写型用例的执行规则以 [SKILL.md](../SKILL.md) 「写型用例三条规则」为唯一权威,本文不复述。

   ⛔ **不得在括号里罗列规则要点标签**(如 `只执行一次` / `不自动重放` / `不自动重试重跑` 之类)—— pre-flight 裁决是"连要点标签也不复述",Task 2 已按**零复述**执行,此处必须同尺度。
...
# 规则要点标签一律不得出现(pre-flight 裁决:连标签也不复述;Task 2 已按零复述执行)
n=$(grep -cE '只执行一次|不自动重试|不自动重跑|不自动重放|只交付不重打' $F)
[ "$n" -eq 0 ] && echo "PASS: 零复述规则要点标签" || echo "FAIL: 复述了规则要点标签(命中 $n 行,应为 0)"
```
